...
static void CheckApiHooks(HWND hwnd, ProcInfo& pi){ if(!g_settings.enableApiHookScan) return; auto &ex=g_extra[pi.pid]; if(ex.apiHooksChecked) return; ex.apiHooksChecked=true; HMODULE ntdll=GetModuleHandleW(L"ntdll.dll"); if(!ntdll) return; HANDLE h=OpenProcess(PROCESS_QUERY_INFORMATION|PROCESS_VM_READ,FALSE,pi.pid); if(!h) return; const char* apis[] = {"NtOpenProcess","NtWriteVirtualMemory","NtCreateThreadEx","NtAllocateVirtualMemory","NtProtectVirtualMemory","NtReadVirtualMemory","NtQuerySystemInformation","NtQueryInformationProcess","NtQueryInformationThread","NtSetInformationThread","NtGetContextThread","NtSetContextThread","NtSuspendThread","NtResumeThread","NtTerminateProcess","NtTerminateThread","NtCreateProcess","NtCreateProcessEx","NtCreateUserProcess","NtOpenThread","NtQueueApcThread","NtQueueApcThreadEx","NtCreateSection","NtOpenSection","NtMapViewOfSection","NtUnmapViewOfSection","NtCreateFile","NtOpenFile","NtReadFile","NtWriteFile","NtDeleteFile","NtSetInformationFile","NtQueryInformationFile","NtQueryDirectoryFile","NtCreateKey","NtOpenKey","NtDeleteKey","NtSetValueKey","NtQueryValueKey","NtEnumerateKey","NtEnumerateValueKey","NtClose","NtDuplicateObject","NtQueryObject","NtSetInformationObject","NtWaitForSingleObject","NtWaitForMultipleObjects","NtDelayExecution","NtYieldExecution","NtCreateToken","NtOpenProcessToken","NtOpenThreadToken","NtAdjustPrivilegesToken","NtSetInformationToken","NtDuplicateToken","NtImpersonateThread","NtCreateSymbolicLinkObject","NtLoadDriver","NtUnloadDriver","NtDeviceIoControlFile","NtFsControlFile","NtLockFile","NtUnlockFile","NtFlushBuffersFile","NtQueryVirtualMemory","NtFreeVirtualMemory","NtCreateThreadEx","NtRaiseHardError"}; for(const char* a: apis){ FARPROC fp=GetProcAddress(ntdll,a); if(!fp) continue; uint8_t buf[32]{}; SIZE_T br=0; if(!ReadProcessMemory(h,fp,buf,sizeof(buf),&br) || br<5) continue; if(!SuspiciousPrologue(buf,br)) continue; if(!AlertAllowed(pi.pid,XDR::EventType::AlertApiHook,(uintptr_t)fp)) continue; std::wstringstream bhex; size_t show=std::min<SIZE_T>(br,16); for(size_t i=0;i<show;i++){ if(i) bhex<<L" "; bhex<<std::hex<<std::uppercase<<std::setw(2)<<std::setfill(L'0')<<(int)buf[i]; } std::wstring apiName; apiName.assign(a,a+strlen(a)); std::wstring disasm=L"unknown"; uintptr_t target=0; bool haveTarget=false; if(buf[0]==0xE9 && br>=5){ int32_t rel=*reinterpret_cast<const int32_t*>(buf+1); target=(uintptr_t)fp+5+rel; disasm=std::format(L"jmp 0x{:X}",target); haveTarget=true; } else if(buf[0]==0xE8 && br>=5){ int32_t rel=*reinterpret_cast<const int32_t*>(buf+1); target=(uintptr_t)fp+5+rel; disasm=std::format(L"call 0x{:X}",target); haveTarget=true; } else if(buf[0]==0xFF && buf[1]==0x25 && br>=6){ int32_t rel=*reinterpret_cast<const int32_t*>(buf+2); uintptr_t rip=(uintptr_t)fp+6; uintptr_t ptrAddr=rip+rel; uintptr_t ptrValue=0; SIZE_T tr=0; if(ReadProcessMemory(h,(LPCVOID)ptrAddr,&ptrValue,sizeof(ptrValue),&tr) && tr==sizeof(ptrValue) && ptrValue){ target=ptrValue; haveTarget=true; disasm=std::format(L"jmp [rip+0x{:X}] -> 0x{:X}",ptrAddr-rip,target); } else disasm=std::format(L"jmp [rip+0x{:X}]",ptrAddr-rip); } else if(buf[0]==0x48 && buf[1]==0xB8 && br>=12){ uint64_t imm=*reinterpret_cast<const uint64_t*>(buf+2); if(br>=12 && buf[10]==0xFF && buf[11]==0xE0){ target=(uintptr_t)imm; disasm=std::format(L"mov rax,0x{:X}; jmp rax",target); haveTarget=true; } else disasm=std::format(L"mov rax,0x{:X}",(uint64_t)imm); } bool rwxTarget=false; std::wstring tgtProt=L"?"; if(haveTarget){ MEMORY_BASIC_INFORMATION mbi{}; if(VirtualQueryEx(h,(LPCVOID)target,&mbi,sizeof(mbi))==sizeof(mbi)){ DWORD p=mbi.Protect; bool exec=(p & (PAGE_EXECUTE|PAGE_EXECUTE_READ|PAGE_EXECUTE_READWRITE|PAGE_EXECUTE_WRITECOPY))!=0; bool write=(p & (PAGE_READWRITE|PAGE_EXECUTE_READWRITE|PAGE_WRITECOPY|PAGE_EXECUTE_WRITECOPY))!=0; if(exec && write) rwxTarget=true; switch(p){ case PAGE_EXECUTE: tgtProt=L"X"; break; case PAGE_EXECUTE_READ: tgtProt=L"RX"; break; case PAGE_EXECUTE_READWRITE: tgtProt=L"RWX"; break; case PAGE_EXECUTE_WRITECOPY: tgtProt=L"WCX"; break; case PAGE_READONLY: tgtProt=L"R"; break; case PAGE_READWRITE: tgtProt=L"RW"; break; case PAGE_WRITECOPY: tgtProt=L"WC"; break; case PAGE_NOACCESS: tgtProt=L"NA"; break; default: tgtProt=L"?"; break; } } } const wchar_t* label = rwxTarget? L"ApiHookSuspicious" : L"ApiHook"; auto line=std::format(L"[{}] ALERT {} pid={} name={} api={} addr=0x{:X} bytes={} {}{}", (long long)duration_cast<seconds>(system_clock::now().time_since_epoch()).count(), label, pi.pid, pi.image, apiName,(uintptr_t)fp,bhex.str(),disasm, rwxTarget?L" (RWX target)":L""); Logger::Write(line); XDR::Event ev; ev.category=XDR::EventCategory::Alert; ev.type=XDR::EventType::AlertApiHook; ev.pid=pi.pid; ev.image=pi.image; ev.details=std::format(L"api={} addr=0x{:X} bytes={} disasm={} target=0x{:X} targetProt={} rwxTarget={}",apiName,(uintptr_t)fp,bhex.str(),disasm,haveTarget?target:0,tgtProt,rwxTarget?1:0); XDR::Storage::Insert(ev); auto* msg=new std::wstring(line); PostMessageW(hwnd,WM_APP+2,(WPARAM)msg,0); } CloseHandle(h); }

static void CheckIATHooks(HWND hwnd, ProcInfo& pi){ if(!g_settings.enableApiHookScan) return; auto &ex=g_extra[pi.pid]; if(ex.iatHooksChecked) return; ex.iatHooksChecked=true; HANDLE h=OpenProcess(PROCESS_QUERY_INFORMATION|PROCESS_VM_READ,FALSE,pi.pid); if(!h) return; HMODULE mods[256]; DWORD need=0; if(!EnumProcessModules(h,mods,sizeof(mods),&need)){ CloseHandle(h); return; } size_t modCnt=std::min<size_t>(need/sizeof(HMODULE),256); for(size_t m=0;m<modCnt;m++){ MODULEINFO mi{}; if(!GetModuleInformation(h,mods[m],&mi,sizeof(mi))) continue; std::vector<uint8_t> headers(4096); SIZE_T br=0; if(!ReadProcessMemory(h,mi.lpBaseOfDll,headers.data(),headers.size(),&br) || br<1024) continue; auto dos=(IMAGE_DOS_HEADER*)headers.data(); if(dos->e_magic!=IMAGE_DOS_SIGNATURE || dos->e_lfanew<0 || dos->e_lfanew>=(int)br-sizeof(IMAGE_NT_HEADERS64)) continue; auto nt=(IMAGE_NT_HEADERS64*)(headers.data()+dos->e_lfanew); if(nt->Signature!=IMAGE_NT_SIGNATURE) continue; bool is64=(nt->FileHeader.Machine==IMAGE_FILE_MACHINE_AMD64); DWORD iatRva=0,iatSize=0; if(is64){ if(nt->OptionalHeader.NumberOfRvaAndSizes>IMAGE_DIRECTORY_ENTRY_IAT){ iatRva=nt->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IAT].VirtualAddress; iatSize=nt->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IAT].Size; } } else { auto nt32=(IMAGE_NT_HEADERS32*)(headers.data()+dos->e_lfanew); if(nt32->OptionalHeader.NumberOfRvaAndSizes>IMAGE_DIRECTORY_ENTRY_IAT){ iatRva=nt32->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IAT].VirtualAddress; iatSize=nt32->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IAT].Size; } } if(!iatRva || !iatSize) continue; uintptr_t iatAddr=(uintptr_t)mi.lpBaseOfDll+iatRva; std::vector<uint8_t> iatData(iatSize); br=0; if(!ReadProcessMemory(h,(LPCVOID)iatAddr,iatData.data(),iatSize,&br) || br<sizeof(uintptr_t)) continue; size_t entryCount=br/sizeof(uintptr_t); auto iatEntries=(uintptr_t*)iatData.data(); for(size_t i=0;i<entryCount;i++){ uintptr_t entry=iatEntries[i]; if(!entry) continue; MEMORY_BASIC_INFORMATION mbi{}; if(VirtualQueryEx(h,(LPCVOID)entry,&mbi,sizeof(mbi))!=sizeof(mbi)) continue; if(mbi.State!=MEM_COMMIT) continue; bool isExec=(mbi.Protect&(PAGE_EXECUTE|PAGE_EXECUTE_READ|PAGE_EXECUTE_READWRITE|PAGE_EXECUTE_WRITECOPY))!=0; if(!isExec) continue; uint8_t code[32]{}; SIZE_T cbr=0; if(!ReadProcessMemory(h,(LPCVOID)entry,code,sizeof(code),&cbr) || cbr<5) continue; if(!SuspiciousPrologue(code,cbr)) continue; uintptr_t iatEntryAddr=iatAddr+i*sizeof(uintptr_t); if(!AlertAllowed(pi.pid,XDR::EventType::AlertApiHook,iatEntryAddr)) continue; std::wstringstream bhex; size_t show=std::min<SIZE_T>(cbr,16); for(size_t j=0;j<show;j++){ if(j) bhex<<L" "; bhex<<std::hex<<std::uppercase<<std::setw(2)<<std::setfill(L'0')<<(int)code[j]; } wchar_t modName[MAX_PATH]=L"unknown"; GetModuleFileNameExW(h,mods[m],modName,MAX_PATH); std::wstring modPath=modName; size_t slash=modPath.rfind(L'\\'); if(slash!=std::wstring::npos) modPath=modPath.substr(slash+1); auto line=std::format(L"[{}] ALERT IATHook pid={} name={} module={} iat_entry=0x{:X} target=0x{:X} bytes={}", (long long)duration_cast<seconds>(system_clock::now().time_since_epoch()).count(), pi.pid, pi.image, modPath, iatEntryAddr, entry, bhex.str()); Logger::Write(line); XDR::Event ev; ev.category=XDR::EventCategory::Alert; ev.type=XDR::EventType::AlertApiHook; ev.pid=pi.pid; ev.image=pi.image; ev.details=std::format(L"module={} iat_entry=0x{:X} target=0x{:X} bytes={}",modPath,iatEntryAddr,entry,bhex.str()); XDR::Storage::Insert(ev); auto* msg=new std::wstring(line); PostMessageW(hwnd,WM_APP+2,(WPARAM)msg,0); } } CloseHandle(h); }
