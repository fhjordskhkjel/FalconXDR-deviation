...
#endif
// Minimal ProcessBasicInformation for CmdLine query
...
static NtQuerySystemInformation_t pNtQuerySystemInformation=nullptr; static NtQueryInformationThread_t pNtQueryInformationThread=nullptr; static NtQueryVirtualMemory_t pNtQueryVirtualMemory=nullptr; static NtQueryInformationProcess_t pNtQueryInformationProcess=nullptr;
...
// Minimal structs to walk PEB -> ProcessParameters -> CommandLine
...
static std::wstring QueryProcessCmdLine(DWORD pid){ if(!pNtQueryInformationProcess){ HMODULE ntdll=GetModuleHandleW(L"ntdll.dll"); if(ntdll) pNtQueryInformationProcess=(NtQueryInformationProcess_t)GetProcAddress(ntdll,"NtQueryInformationProcess"); }
    if(!pNtQueryInformationProcess) return L""; std::wstring out; HANDLE h=OpenProcess(PROCESS_QUERY_INFORMATION|PROCESS_VM_READ,FALSE,pid); if(!h) return out; PROCESS_BASIC_INFORMATION_LITE pbi{}; ULONG ret=0; NTSTATUS st=pNtQueryInformationProcess(h,ProcessBasicInformation,&pbi,sizeof(pbi),&ret); if(!NT_SUCCESS(st) || !pbi.PebBaseAddress){ CloseHandle(h); return out; } PEB_LITE peb{}; SIZE_T br=0; if(!ReadProcessMemory(h,pbi.PebBaseAddress,&peb,sizeof(peb),&br) || !peb.ProcessParameters){ CloseHandle(h); return out; } RTL_USER_PROCESS_PARAMETERS_LITE upp{}; if(!ReadProcessMemory(h,peb.ProcessParameters,&upp,sizeof(upp),&br) || upp.CommandLine.Length==0 || !upp.CommandLine.Buffer){ CloseHandle(h); return out; } std::vector<wchar_t> buf((upp.CommandLine.Length/sizeof(wchar_t))+1); SIZE_T rd=0; if(ReadProcessMemory(h,upp.CommandLine.Buffer,buf.data(),upp.CommandLine.Length,&rd) && rd){ buf[rd/sizeof(wchar_t)]=0; out.assign(buf.data()); } CloseHandle(h); return out; }
...
struct ProcInfo { DWORD pid; std::wstring image; steady_clock::time_point start; DWORD parentPid{}; std::wstring integrity; std::wstring parentIntegrity; bool seDebug=false; bool adminGroup=false; bool privAlerted=false; bool followPriv=false; bool injAlerted=false; bool hollowAlerted=false; bool reflMemAlerted=false; bool checkedModules=false; bool hasDbgHelp=false; bool hasComSvcs=false; bool hasLsass=false; std::wstring lastIntegrityDyn; std::set<std::wstring> privSnapshot; std::unordered_map<uintptr_t,DWORD> lastProt; std::unordered_set<DWORD> knownThreads; std::vector<std::pair<uintptr_t,uintptr_t>> moduleRanges; steady_clock::time_point lastModEnum{}; steady_clock::time_point nextRegionScan{}; std::wstring userSid; std::unordered_set<DWORD> alertedImpersonatingThreads; 
    // New: per-process scan scheduling
    steady_clock::time_point nextExecClassify{}; 
    steady_clock::time_point nextInjHeur{}; int injHeurRuns=0; 
    steady_clock::time_point nextReflCheck{}; int reflRuns=0; 
    steady_clock::time_point nextHollowCheck{}; int hollowRuns=0; 
    // New: parent tracking
    std::wstring parentImage; bool parentOpened=false; };
...
static std::mutex g_yaraM; static std::vector<YaraTask> g_yaraTasks; static std::atomic_bool g_yaraRun{false}; static std::thread g_yaraThread; static uint64_t g_yaraByteWindow=0; struct YWin{ steady_clock::time_point ts; size_t bytes; }; static std::deque<YWin> g_yWin; static const size_t kYaraBytesMin=8*1024*1024; 
...
static void EnqueueYara(DWORD pid, uintptr_t base,size_t size,const std::wstring& ctx){ if(!g_settings.enableYaraRegionScan) return; size=std::min<size_t>(size,g_settings.yaraMaxRegionSize); std::lock_guard lk(g_yaraM); if(g_yaraTasks.size()>512) return; 
    if(CountQueuedForPid(pid)>=kYaraPerPidMaxQueued) return; 
    TrimPidYWin(pid); auto cur=g_yaraPidByteWindow[pid]; if(cur + size > kYaraPerPidBytesMin) {
...
static void DetectParentChildAnomaly(HWND hwnd, const ProcInfo& pi, const std::wstring& childImage){
    std::wstring parent=pi.parentImage; std::wstring child=childImage; std::wstring lp=lower(parent); std::wstring lc=lower(child);
    // Office -> shells
    ...
    // PPID spoofing heuristic: no parent
...
namespace Behavioral {
    ...
    void OnProcessStart(DWORD pid,const std::wstring& image,HWND hwnd){
        ...
        // initialize per-process scan scheduling
        ...
        pi.nextExecClassify = now + seconds(d1); // initial delay fraction
        ...
        // Parent-child anomalies and cmdline checks
    ...
    void Periodic(HWND hwnd){
        ...
        for(auto &kv: g_procs){
            ...
            // Injection heuristic (rate-limited)
            ...
            // Hollowing / reflective detection (rate-limited)
            ...
            // Exec region classification (rate-limited)
