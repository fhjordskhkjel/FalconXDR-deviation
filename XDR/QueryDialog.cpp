...
struct QueryState { uint64_t lastId{0}; std::optional<XDR::EventCategory> category; std::wstring filter; std::optional<uint32_t> pid; bool live{true}; HWND list{nullptr}; };
...
static void FormatAndAdd(QueryState* qs, const XDR::Event &ev){
    ...
    std::wstring line = std::format(L"#{:06} {:>5} {} {:>5} {} {}", ev.id, ev.pid, ToString(ev.category), (int)ev.type, ToString(ev.type), ev.details);
