...
static void Layout(HWND hwnd){ RECT rc; GetClientRect(hwnd,&rc); int w=rc.right, h=rc.bottom; int top=kBarH+kSearchH+4, bottom=h-kStatusH; if(g_toolbar) SetWindowPos(g_toolbar,nullptr,0,0,w,kBarH,SWP_NOZORDER); int m=6; if(g_searchEdit) SetWindowPos(g_searchEdit,nullptr,m,kBarH+4,240,kSearchH-8,SWP_NOZORDER); if(g_searchBtn) SetWindowPos(g_searchBtn,nullptr,m+244,kBarH+4,70,kSearchH-8,SWP_NOZORDER); if(g_clearBtn) SetWindowPos(g_clearBtn,nullptr,m+244+74,kBarH+4,70,kSearchH-8,SWP_NOZORDER); if(g_splitter) SetWindowPos(g_splitter,nullptr,g_uiSettings.splitPos,top,kSplitterW,bottom-top,SWP_NOZORDER); if(g_lvEvents) SetWindowPos(g_lvEvents,nullptr,2,top,g_uiSettings.splitPos-2,bottom-top,SWP_NOZORDER); if(g_lvAlerts) SetWindowPos(g_lvAlerts,nullptr,g_uiSettings.splitPos+kSplitterW,top,w-g_uiSettings.splitPos-kSplitterW-2,bottom-top,SWP_NOZORDER); if(g_status){ SetWindowPos(g_status,nullptr,0,bottom,w,kStatusH,SWP_NOZORDER); int parts[5]; parts[0]=250; parts[1]=470; parts[2]=720; parts[3]=920; parts[4]=-1; SendMessageW(g_status,SB_SETPARTS,5,(LPARAM)parts);} } 
static void UpdateStatus(){ if(!g_status) return; std::wstring counts=std::format(L"Events: {} | Alerts: {}",g_evtCount.load(),g_alertCount.load()); std::wstring state=!XDR::g_collect.load()?L"⏸ STOPPED":(g_pause?L"⏸ DISPLAY PAUSED":L"▶ COLLECTING"); auto qstats=Behavioral::GetQueueStats(); std::wstring queues=std::format(L"Q: Scan:{} Y:{} H:{} C:{}",qstats.scanQueueSize,qstats.yaraQueueSize,qstats.scanQueueHighPrio,qstats.scanQueueCriticalPrio); std::wstring driver=std::format(L"Driver: {}",g_driverMgr.IsActive()?L"✓ Active":L"○ Inactive"); std::wstring theme=std::format(L"Theme: {}",g_theme.name); SendMessageW(g_status,SB_SETTEXTW,0,(LPARAM)counts.c_str()); SendMessageW(g_status,SB_SETTEXTW,1,(LPARAM)state.c_str()); SendMessageW(g_status,SB_SETTEXTW,2,(LPARAM)queues.c_str()); SendMessageW(g_status,SB_SETTEXTW,3,(LPARAM)driver.c_str()); SendMessageW(g_status,SB_SETTEXTW,4,(LPARAM)theme.c_str()); }
