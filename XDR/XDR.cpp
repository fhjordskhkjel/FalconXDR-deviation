...
#include "DriverManager.h" // fixed include extension
...
// Severity colors
static COLORREF g_sevHigh=RGB(255,90,90), g_sevMed=RGB(255,170,60), g_sevLow=RGB(255,220,60);
...
// Determine severity for alert line
static COLORREF GetSeverityColor(const std::wstring& line){ if(line.find(L"ProcessInjection")!=std::wstring::npos||line.find(L"ProcessHollowing")!=std::wstring::npos||line.find(L"DllInjection")!=std::wstring::npos||line.find(L"ReflectiveModule")!=std::wstring::npos||line.find(L"ReflectiveMemory")!=std::wstring::npos||line.find(L"PrivilegedExec")!=std::wstring::npos) return g_sevHigh; if(line.find(L"SuspiciousProcess")!=std::wstring::npos||line.find(L"SuspiciousRemotePort")!=std::wstring::npos||line.find(L"ProcEnumMismatch")!=std::wstring::npos) return g_sevMed; if(line.find(L"YaraMatch")!=std::wstring::npos||line.find(L"ScreenshotCaptured")!=std::wstring::npos) return g_sevLow; return g_theme.alertText; }
